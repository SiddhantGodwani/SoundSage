...
def get_mfcc_features(track_preview_path):
...
def get_recommendations(track_name):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        track_mfcc_features = get_mfcc_features(track_preview_path) if track_preview_path else None
        ...
        for seed_track in [track_uri]:
            ...
            for track in recs:
                ...
                if preview_path:
                    mfcc_features = get_mfcc_features(preview_path)
                    similarity = np.linalg.norm(track_audio_features - audio_features) + np.linalg.norm(track_mfcc_features - mfcc_features)
                else:
