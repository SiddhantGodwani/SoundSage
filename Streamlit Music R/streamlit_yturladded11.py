# AIzaSyC7vUv6GFbU_-8fHe1llSAz-moq3gDykgE    <<<<<<<<<<<<<<<<<----key
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for seed_track in [track_uri]:
                ...
                for track in recs:
                    ...
                    recommendations[track_language].append((track, similarity, preview_url, track_url, youtube_video))

            # Sort recommendations by language and similarity
            for language, recs in recommendations.items():
                recs.sort(key=lambda x: x[1])
                recommendations[language] = [(rec[0], rec[2], rec[3], rec[4]) for rec in recs[:10]]
...
if track_name:
    ...
    for language, recs in recommendations.items():
        ...
        for track, preview_url, spotify_url, youtube_url in recs:
