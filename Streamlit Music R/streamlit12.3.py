...
#A radar chart visualizing the audio features of the recommended songs.
#The ability for users to select tracks and create a Spotify playlist from their selections.
...
redirect_uri = "http://localhost:8501"  # This should match your Spotify app settings
...
def get_spotify_auth():
    ...
    if not auth_manager.get_cached_token():
        ...
        st.write(f"Please [log in to Spotify]({auth_url}) and paste the redirect URL below:")
        redirected_url = st.text_input("Enter the URL you were redirected to:")
        if redirected_url:
            auth_manager.get_access_token(redirected_url)
    
    return spotipy.Spotify(auth_manager=auth_manager)


def create_spotify_playlist(track_uris, playlist_name):
    try:
        sp = get_spotify_auth()
        user_id = sp.me()['id']
...
def display_recommendations(recommendations):
    ...
    for language, recs in recommendations.items():
        ...
        for i, (track, preview_url, spotify_url, youtube_url, image_url) in enumerate(recs):
            with st.expander(f"{track['name']} - {track['artists'][0]['name']}"):
                ...
                if st.checkbox(f"Add to playlist", key=f"playlist_{track['id']}_{i}"):
    ...
    if selected_tracks:
        ...
        if st.button("Create Playlist"):
            playlist_url = create_spotify_playlist(selected_tracks, playlist_name)
            if playlist_url:
