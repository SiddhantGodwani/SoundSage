    ...
    for song_id in liked_songs:
        ...
        if preview_url:
            try:
                ...
                with open(f"temp_audio_{song_id}.mp3", "wb") as f:
                    f.write(response.content)

                # Load the downloaded audio file using librosa
                y, sr = librosa.load(f"temp_audio_{song_id}.mp3", offset=10, duration=30)
                ...
                os.remove(f"temp_audio_{song_id}.mp3")
            except Exception as e:
                print(f"Error processing '{track['name']}' by {track['artists'][0]['name']}: {e}")
        else:
            # Fill in missing values with 0
            feature_vector = [np.zeros(20), np.zeros(7), np.zeros(12)]
            feature_vector.extend([0] * 11)
    ...
    for track in recommendations:
        ...
        if preview_url:
            try:
                # Download the audio file from the preview URL
                response = requests.get(preview_url)
                with open(f"temp_audio_{track['id']}.mp3", "wb") as f:
                    f.write(response.content)

                # Load the downloaded audio file using librosa
                y, sr = librosa.load(f"temp_audio_{track['id']}.mp3", offset=10, duration=30)
                ...
                # Remove the downloaded audio file
                os.remove(f"temp_audio_{track['id']}.mp3")
            except Exception as e:
                print(f"Error processing '{track['name']}' by {track['artists'][0]['name']}: {e}")
        else:
